...
목록과 상세 화면은 `data/card-images.json`에 기록한 카드사 공식 이미지를 표시합니다. 현재 344개 상품 중 335개를 복구했고, 원본 다운로드 실패 또는 상품 식별이 일치하지 않는 9개는 `이미지 준비 중`으로 표시합니다. 이미지는 `site/images/cards/`에 원본 그대로 저장하며, 이미지 URL·상품 출처·확인일·SHA-256을 함께 남깁니다. 카드 디자인의 권리는 각 카드사에 있습니다.
